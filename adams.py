...
class VariableCoefficientAdamsBashforth(AdaptiveStepsizeODESolver):
    ...
    def before_integrate(self, t):
        ...
        assert first_step > 0, "initial step size must be positive t0 = {}, first_step = {}".format(t0, first_step)
    ...
    def advance(self, final_t):
        ...
        while final_t > self.vcabm_state.prev_t[0]:
            print('before _adaptive_adams_step')
...
class VariableCoefficientJumpAdamsBashforth(VariableCoefficientAdamsBashforth):
    ...
    def _adaptive_adams_step(self, vcabm_state, final_t):
        ...
        # during backprop, step then jump
            ...
            if prev_t[0] != t0:  # did perform a step
                dy = self.func.simulated_jump(dN, prev_t[0], y1)
            else:
                dy = None
        else:
            dy = None
