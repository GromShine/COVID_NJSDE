...
parser.add_argument('--dataset', type=str, default='m2')
...
if __name__ == '__main__':
    ...
    A_true = np.loadtxt("A_true.txt",delimiter=",")
    ...
    if func.jump_type == "read":
        while it < args.niters:
            ...
            print('difference', np.linalg.norm(B_matrix.detach().numpy()-A_true))
            if it % 1 == 0:
                plt.figure(1)
                #plt.imshow(A_matrix.detach().numpy(), cmap='hot', interpolation='nearest', vmin=0, vmax=0.05)
                plt.imshow(B_matrix.detach().numpy(), cmap='hot', interpolation='nearest', vmin=0, vmax=2)
                plt.pause(0.5)
